...
class SoundData(Dataset):
    def __init__(self, f_p, f_l=0.5, f_h=500, s_r=4000, m_s=10, sr=4000):
        """
    ...
    def resample_signal(self, file, new_sr, old_sr):
        """
        parameter:
    ...
    def band_pass_filter(self, waveform):
        ...
        for i in range(num_channels):
            waveform[i] = torch.tensor(signal.lfilter(b, a, waveform[i]))
        return waveform
    ...
    def __getitem__(self, idx):
        ...
        processed_waveform = self.resample_signal(waveform, new_sr=self.sr, old_sr=sample_rate)
        processed_waveform = self.band_pass_filter(processed_waveform)
...
if __name__ == '__main__':
    data_folder_path = "the-circor-digiscope-phonocardiogram-dataset-1.0.3//training_data"
